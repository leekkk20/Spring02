...
public class Ex01_SpringDAOTest {
	// ** 자동 주입 : 생성은 root~~~.xml로 설정
	@Autowired
	MemberDAO dao;
	@Autowired
	MemberVO vo;
	
	// test1) 자동 주입과 Detail 구현 Test
	// => DAO 자동 주입 확인
	// => Detail 구현 : 정확한 id는 NotNull : green, 없는 id는 Null : red(오류)
	public void detailTest() {
		
		// 1.1) DAO 자동 주입 확인
		System.out.println("** DAO 자동 주입 확인 => " + dao);
		assertNotNull(dao);
		
		
		// 1.2) Detail 구현
		 vo.setId("ggumsb"); // Not_Null : Green
//		vo.setId("aaaaa");		// Null : Red
